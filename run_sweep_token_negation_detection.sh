#!/bin/bash
# =============================================================================
# Sweep: TOKEN Pooling - Negation Detection Task
# =============================================================================
...
# TOKEN pooling focuses on the "not" token position
#
# Run this in one Colab runtime while running cls/mean in others
#
# For Google Drive persistence, set DRIVE_OUTPUT before running:
#   export DRIVE_OUTPUT="/content/drive/MyDrive/NOT_results"
#   ./run_sweep_token_negation_detection.sh
...
if [ -n "$DRIVE_OUTPUT" ]; then
    OUTPUT_DIR="$DRIVE_OUTPUT/sweep_token_negation_detection"
    echo "Saving to Google Drive: $OUTPUT_DIR"
else
    OUTPUT_DIR="${OUTPUT_DIR:-experiments/sweep_token_negation_detection}"
    echo "Saving locally: $OUTPUT_DIR"
fi

echo "========================================"
echo "Layer Sweep: TOKEN Pooling - NEGATION DETECTION"
echo "========================================"
echo "Task: Binary classification (0=no negation, 1=has negation)"
echo "Pooling: Around 'not' token position"
echo "Layers: 0-5"
...
RESULTS_FILE="$OUTPUT_DIR/results_token_negation_detection.json"
echo "[]" > "$RESULTS_FILE"

for layer in 0 1 2 3 4 5; do
    echo ""
    echo "[Layer $layer] TOKEN pooling - Negation Detection..."
    LOG="$OUTPUT_DIR/logs/layer${layer}_token.log"
    CKPT_DIR="$OUTPUT_DIR/checkpoints/layer${layer}_token"
    mkdir -p "$CKPT_DIR"
...
"
        ...
        echo "  ✓ Layer $layer TOKEN: acc=$ACC, auroc=$AUROC"
    else
        echo "  ✗ Layer $layer TOKEN: failed"
    fi
...
echo ""
echo "========================================"
echo "TOKEN Negation Detection Sweep Complete!"
echo "========================================"
