"""
Fast automated layer search script.
Trains all pooling strategies for each layer sequentially (no multiprocessing).
More efficient than parallel version - reuses data loading and avoids process overhead.
"""
...
def train_probe_for_layer(
    ...
    args,
    base_output_dir: str,
    datamodule: NOTDataModule = None
):
    """
    Train a probe for a specific layer and pooling strategy.
    Can reuse datamodule if provided.
    """
    ...
    # Initialize or reuse data module
...
def main():
    parser = argparse.ArgumentParser(
        description="Fast automated layer search: train all pooling strategies per layer"
    )
    ...
    # Check for partially completed experiments and prioritize them
    partially_completed = []
    not_started = []
    ...
    if remaining == 0:
        ...
        # Initialize datamodule once (will be reused)
        print("Initializing datamodule...")
        datamodule = NOTDataModule.from_args(args)
        print("✓ Datamodule initialized\n")
        
        # Group experiments by layer for efficient processing
        ...
        # Process each layer with all its pooling strategies
        for layer_idx in sorted(experiments_by_layer.keys()):
            pooling_strategies_for_layer = experiments_by_layer[layer_idx]
            
            print(f"\n{'='*60}")
            print(f"Layer {layer_idx}: Training {len(pooling_strategies_for_layer)} pooling strategies")
            print(f"{'='*60}")
            
            for pooling_strategy in pooling_strategies_for_layer:
                ...
                try:
                    result = train_probe_for_layer(
                        ...
                        datamodule=datamodule  # Reuse datamodule
                    )
